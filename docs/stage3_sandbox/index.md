...
* **核心術語與彼此關聯**：
  * **Image (映像檔)**：就像是**遊戲安裝光碟**或是**建築藍圖**。它是一個唯讀的範本，裡面已經封裝了作業系統、依賴套件以及我們的 AUV 程式碼。我們無法直接在光碟上玩遊戲或修改它。
  * **Container (容器)**：就像是**把光碟安裝到電腦後真正跑起來的遊戲實體**，或是**根據藍圖實際蓋出來的房子**。Container 是由 Image 建立出來的運行環境。我們可以在 Container 裡面執行程式、讀寫檔案，且它與外面的宿主電腦環境是完全隔離的安全沙盒。同一個 Image 可以同時啟動無限多個相互獨立的 Container。
  * **Dockerfile**：就像是**製作遊戲光碟的自動化腳本**或是**料理食譜**。它是一個純文字檔，裡面寫滿了指令（例如第一步要裝 Ubuntu 系統，第二步要裝 Python，第三步要把程式碼放進去）。Docker 引擎會讀取 Dockerfile 並依照步驟「自動烘焙」出我們的 Image。
  * **Docker Compose**：隨著專案變複雜，我們可能需要一個容器跑資料庫、一個跑視覺辨識、一個跑控制節點。Docker Compose 就像是**交響樂團指揮家**。透過一個 `docker-compose.yml` 設定檔，我們只需下一行指令，就能同時啟動並管理多個互相關聯的 Container，並自動設定好它們之間的內部網路連線，是系統整合階段必備的工具。
  * **Volume (資料卷映射)**：將本機的資料夾掛載到容器內部，讓本地端的檔案能與容器內即時同步，方便直接修改代碼而不需重新打包 Image。
